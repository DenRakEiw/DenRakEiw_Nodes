...
class LoadImageSequence:
        ...
        return {
            ...
            "optional": {
                ...
                "seed": ("INT", {
                    ...
                    "tooltip": "Nur bei random_image: fester Seed = reproduzierbare Auswahl. "
                               "0 = echter Zufall"
                }),
            }
        }
    ...
    def _pick_random_index(self, state, image_files, no_repeats, seed):
        """Pick a random index, optionally exhausting the folder before repeating."""
        rng = random.Random(seed) if seed else random

        if not no_repeats:
            return rng.randrange(len(image_files))

        # Shuffle bag: every image is shown once per round, then the bag refills.
        pool = state.get('random_pool') or []
        # Files may have been added/removed since the bag was filled.
        pool = [f for f in pool if f in image_files]
        if not pool:
        ...
        return image_files.index(chosen)

    def load_next_image(self, folder_path, reset_sequence, file_extensions="jpg,jpeg,png,bmp,tiff,webp", sort_method="alphabetical", loop_sequence=True, random_image=False, no_repeats=True, seed=0):
        """
        ...
        try:
            ...
            new_execution = current_time - state.get('last_execution', 0) > 0.1  # 100ms threshold

            if random_image:
                if new_execution:
                    state['last_execution'] = current_time
                    current_index = self._pick_random_index(state, image_files, no_repeats, seed)
                    state['current_index'] = current_index
                else:
                    # Same execution: keep the image we already picked, don't re-roll.
            ...
            if current_index >= len(image_files):
                if loop_sequence or random_image:
                    current_index = 0
            ...
            if random_image:
                rest = len(state.get('random_pool', [])) if no_repeats else None
                mode = "random"
                if no_repeats:
                    mode += f", noch {rest} im Pool"
                if seed:
                    mode += f", seed {seed}"
                print(f"✓ Loading image {current_index + 1}/{len(image_files)} [{mode}]: {current_file}")
            else:
